...
//        try (Connection conn = DBConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement("select m.matchId, m.date, m.time, (SELECT t.teamName FROM team t where t.teamId = m.team1 ) AS team1, (SELECT t.teamName FROM team t where t.teamId = m.team2 ) AS team2 from `match` m where m.matchId = ?");) {
    ...
    public static HashMap<Integer, Match> getPastMatches(int leagueId, String username) {
...
//and TIMESTAMPDIFF(day, date, date(NOW())) <= 7 
        ...
        //try (Connection conn = DBConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement("select m.matchId,m.tournamentId, m.date, m.time, (SELECT t.teamName FROM team t where t.teamId = m.team1 ) AS team1, (SELECT t.teamName FROM team t where t.teamId = m.team2 ) AS team2, m.team1_score,m.team2_score, coalesce(ml.team1_prediction,-1), coalesce(ml.team2_prediction,-1), (SELECT l.pointsAllocated FROM league l where leagueId = " + leagueId + " ) AS points from log l, `match` m left outer join matcheslog ml on m.matchId = ml.matchId where date < DATE(NOW()) AND (date >= (SELECT startDate FROM `privateleague` where leagueKeyId = " + leagueId + ") AND date <= (SELECT endDate FROM `privateleague` where leagueKeyId = " + leagueId  + ")) AND (team1 IN (SELECT teamId from privateleagueteams where leagueId = " + leagueId + ") OR team2 IN (SELECT teamId from privateleagueteams where leagueId = " + leagueId + ")) AND (m.team1_score != -1 OR m.team2_score != -1) and l.username = '"+ username+"' and l.leagueId = "+ leagueId +"  order by m.date desc limit 10 ");) {
        try (Connection conn = DBConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement("select m.matchId,m.tournamentId, m.date, m.time, (SELECT t.teamName FROM team t where t.teamId = m.team1 ) AS team1, (SELECT t.teamName FROM team t where t.teamId = m.team2 ) AS team2, m.team1_score,m.team2_score, coalesce(matches.team1_prediction,-1), coalesce(matches.team2_prediction,-1), (SELECT l.pointsAllocated FROM league l where leagueId = " + leagueId + " ) AS points from `match` m left join (select ml.team1_prediction, ml.team2_prediction, ml.matchId, l.username from matcheslog ml inner join log l on l.logId = ml.logId and username = '" + username + "') AS matches on m.matchId = matches.matchId where date < DATE(NOW()) AND (date >= (SELECT startDate FROM privateleague where leagueKeyId = " + leagueId + ") AND date <= (SELECT endDate FROM privateleague where leagueKeyId = " + leagueId + ")) AND (team1 IN (SELECT teamId from privateleagueteams where leagueId = " + leagueId + ") OR team2 IN (SELECT teamId from privateleagueteams where leagueId = " + leagueId + ")) AND (m.team1_score != -1 OR m.team2_score != -1) order by m.date desc limit 10 ");) {
            ResultSet rs = stmt.executeQuery();
            System.out.println(stmt);
            while (rs.next()) {
                int matchID = rs.getInt(1);
